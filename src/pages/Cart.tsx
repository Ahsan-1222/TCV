...
export const Cart = () => {
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 md:px-8 lg:px-12 py-12">
      <h1 className="font-display text-[36px] leading-none">Shopping Bag</h1>
      <div className="grid lg:grid-cols-[1fr_380px] gap-12 mt-10">
        <div className="space-y-6">
          {items.map(item => (
            <div key={item.product.id} className="flex gap-6 border p-4 bg-white">
              <Link to={`/product/${item.product.slug}`} className="w-24 h-28 bg-[#F8F6F3] flex-shrink-0"><img src={(item.product.images.find(i=>i.isMain)||item.product.images[0]).url} alt={item.product.name} className="w-full h-full object-cover" /></Link>
              <div className="flex-1">
                <div className="flex justify-between"><Link to={`/product/${item.product.slug}`} className="font-display uppercase tracking-wide text-[15px]">{item.product.name}</Link><button onClick={()=>removeFromCart(item.product.id)} className="opacity-40 hover:opacity-100"><Trash2 size={14} /></button></div>
                <p className="text-[12px] opacity-60 mt-1">{item.product.shortDescription}</p>
                <div className="flex items-center justify-between mt-4"><div className="flex items-center border"><button onClick={()=>updateQuantity(item.product.id, item.quantity-1)} className="w-8 h-8 flex items-center justify-center hover:bg-black hover:text-white"><Minus size={12}/></button><span className="w-10 text-center text-[13px]">{item.quantity}</span><button onClick={()=>updateQuantity(item.product.id, item.quantity+1)} className="w-8 h-8 flex items-center justify-center hover:bg-black hover:text-white"><Plus size={12}/></button></div><span className="font-semibold">{formatPrice(item.product.price*item.quantity)}</span></div>
              </div>
            </div>
          ))}
        </div>

        <div className="bg-[#0A0A0A] text-white p-8 h-fit sticky top-28 space-y-6">
